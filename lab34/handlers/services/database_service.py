...
def register_city(message):
    try:
        ...
        db.session.commit()
        bot.send_message(message.chat.id, f'{city.city_name} {CITY_ADDED_MESSAGE}')
    except pyowm.commons.exceptions.NotFoundError or pyowm.commons.exceptions.TimeoutError:
        bot.send_message(message.chat.id, INVALID_CITY_MESSAGE)


@bot.callback_query_handler(func=lambda call: call.data.startswith(REMOVE_CITY))
def delete_city(callback):
    ...
    delete_by_callback(callback, cites, text, markup, CITY_DELETED_MESSAGE)
